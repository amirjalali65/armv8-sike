...
/*   asm volatile(
    ...
    // The above version is correct and works with -O0



void mp_mul_comba(digit_t* a, digit_t* b, digit_t* c)
...
asm(
        ...
        "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x29"
    );
}

void fpmul2x384_mixed_arm64(uint64_t *a, uint64_t *b, uint64_t *c, uint64_t *aplusbplus)
{
// The above version is correct and working with -O0 flag in gcc 
// New version
  asm volatile(
        "ldp x3, x4, [%0, #48]      \n\t"
        "ldp x5, x6, [%0, #64]      \n\t"
...
void fpmul768_karatsuba(uint64_t *a, uint64_t *b, uint64_t *c)
{
    ...
    //fpmul448_arm64(aplus, bplus, rplus);
