...
*
* Abstract: API header file for P964
*********************************************************************************************/  

#ifndef __P964_API_H__
#define __P964_API_H__
...
// Encoding of keys for KEM-based isogeny system "SIKEp964" (wire format):
// ----------------------------------------------------------------------
// Elements over GF(p964) are encoded in 94 octets in little endian format (i.e., the least significant octet is located in the lowest memory address). 
// Elements (a+b*i) over GF(p964^2), where a and b are defined over GF(p964), are encoded as {a, b}, with a in the lowest memory portion.
//
...
// Public keys pk consist of 3 elements in GF(p964^2). In the SIKE API, pk is encoded in 564 octets. 
...
// Output: the public key PublicKeyA consisting of 3 GF(p964^2) elements encoded in 564 bytes.
...
// The public key consists of 3 GF(p964^2) elements encoded in 564 bytes.
...
//         Bob's PublicKeyB consists of 3 GF(p964^2) elements encoded in 564 bytes.
// Output: a shared secret SharedSecretA that consists of one element in GF(p964^2) encoded in 188 bytes.
...
//         Alice's PublicKeyA consists of 3 GF(p964^2) elements encoded in 564 bytes.
// Output: a shared secret SharedSecretB that consists of one element in GF(p964^2) encoded in 188 bytes. 
...
// Encoding of keys for KEX-based isogeny system "SIDHp964" (wire format):
// ----------------------------------------------------------------------
// Elements over GF(p964) are encoded in 94 octets in little endian format (i.e., the least significant octet is located in the lowest memory address). 
// Elements (a+b*i) over GF(p964^2), where a and b are defined over GF(p964), are encoded as {a, b}, with a in the lowest memory portion.
//
...
// Public keys PublicKeyA and PublicKeyB consist of 3 elements in GF(p964^2). In the SIDH API, they are encoded in 564 octets. 
// Shared keys SharedSecretA and SharedSecretB consist of one element in GF(p964^2). In the SIDH API, they are encoded in 188 octets.
